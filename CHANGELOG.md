...
### Added — "Watch it prove itself" (the proof loop, with real output)
New README section directly under the plan: seed → ask → audit, using **genuine output generated by running the shipped seeder + search against this repository's own git history** (seed report incl. 7 real redactions — one shown verbatim from a stored row — and a dated, id-addressed answer with its decision surfaced). Ends with the audit affordances (`Show Current Repo Memory`, `/why`, `/compliance`) and an explicit "if this doesn't reproduce, file a bug" invitation.

### Fixed — every stale count claim
The docs simultaneously claimed **597**, **597**, **386**, and (in the comparison table) **597** tests while the suite has **624**; the README claimed 41 chat commands while the registry holds **42**. All corrected. (The comparison table's "231 tests" belongs to a competitor's column and was left alone.)

### Added — count-claim drift gate
`check:release` now extracts every "N tests" claim from README.md + docs/DEMO.md and **fails unless they all agree** — update one count and the gate forces you to update them all. It runs doc-only (no compile), so it guards cross-doc agreement; the release CI run that precedes it verifies the number is real by running the suite.

### Test count
624 tests, unchanged (and now impossible to misquote in two places at once).
