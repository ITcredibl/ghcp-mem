...
const { runEvalSuite, runGoldEval } = require(path.resolve(__dirname, '..', 'out-test', 'src', 'eval.js'));
...
(async () => {
  ...
  for (const run of report.runs) {
    ...
    console.log(`  [${tag}] ${run.label}: recall@${run.k}=${run.recall.toFixed(3)} (floor ${recallMin.toFixed(3)}) · MRR=${run.mrr.toFixed(3)} (floor ${mrrMin.toFixed(3)}) · nDCG@${run.k}=${run.ndcg.toFixed(3)}${floor.ndcg !== undefined ? ` (floor ${ndcgMin.toFixed(3)})` : ''}`);
    if (!ok) failed = true;
  }
  if (failed) {
