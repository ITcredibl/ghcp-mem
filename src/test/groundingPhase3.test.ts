...
test('sessionTouchesEntity — symbol kind matches via evidence symbolId', () => {
  const ev: Evidence[][] = [[{ kind: 'file_edit', filePath: 'src/auth.ts', symbolId: 'src/auth.ts#hashPassword' }]];
...
test('buildEntityRecord — aggregates decisions, problems, topics across sessions', () => {
  ...
  const sessions = [
    makeSession({
      id: 's1', summary: 'auth jwt rework',
      keyFiles: ['src/auth.ts'], keyTopics: ['authentication', 'jwt'],
      decisions: ['use JWT'], problemsSolved: [], endTime: now - 100_000,
    }),
    makeSession({
      id: 's2', summary: 'bcrypt cost bump',
      keyFiles: ['src/auth.ts'], keyTopics: ['authentication', 'security'],
      decisions: ['bcrypt cost 12'], problemsSolved: ['weak hash'], endTime: now,
    }),
    makeSession({
      id: 'other', summary: 'ui tweak',
      keyFiles: ['src/widget.tsx'], keyTopics: ['ui'],
    }),
  ];
  ...
  assert.deepEqual(rec!.decisions.map(d => d.text), ['bcrypt cost 12', 'use JWT'], 'newest decision first');
...
test('renderEntityMarkdown — emits headers and decisions', () => {
  const sessions = [
    makeSession({ id: 's1', keyFiles: ['src/a.ts'], decisions: ['use bcrypt'], keyTopics: ['security'] }),
  ];
...
test('walkSupersedesChain — returns oldest → newest order', () => {
  ...
  map.set('a', a); map.set('b', b); map.set('c', c);
  assert.deepEqual(walkSupersedesChain('c', map).map(s => s.id), ['a', 'b', 'c']);
});
...
test('ContextStore.enrichWithMultiHop — attaches lineage + related entities', async () => {
  ...
  const ev: Evidence[][] = [[{ kind: 'file_edit', filePath: 'src/auth.ts', symbolId: 'src/auth.ts#hashPassword' }]];
  await store.addSession(makeSession({ id: 'old', summary: 'old auth decision', supersededBy: 'new' }));
  await store.addSession(makeSession({
    id: 'new', summary: 'new auth decision',
    supersedes: 'old',
    decisionEvidence: ev,
  ...
  assert.deepEqual(enriched[0].lineage.map(s => s.id), ['old', 'new']);
...
test('effectiveConfidence — old session caps haircut at MAX_DECAY_HAIRCUT', () => {
  ...
  assert.ok(eff >= 1.0 * (1 - MAX_DECAY_HAIRCUT) - 0.001, `eff ${eff} should be ≥ ${1.0 * (1 - MAX_DECAY_HAIRCUT)}`);
...
test('effectiveConfidence — recent retrieval resets decay clock', () => {
  ...
  const refreshedS = makeSession({
    confidence: 1.0, endTime: oldEndTime,
...
test('lastInteractionTimestamp — takes max of endTime + usage timestamps', () => {
  ...
  const s = makeSession({
    endTime: baseEnd,
    usage: { retrieved: 1, lastRetrievedAt: 0, accepted: 1, rejected: 0, lastInteractionAt: interactionTime },
  });
...
test('runGoldEval — produces three runs with finite metrics on a seeded store', async () => {
  const mem = new InMemoryMemento() as any;
  const store = new ContextStore(mem);
  await store.addSession(makeSession({ id: 's1', summary: 'auth refactor', keyTopics: ['authentication'] }));
  await store.addSession(makeSession({ id: 's2', summary: 'payments rework', keyTopics: ['payments'] }));
